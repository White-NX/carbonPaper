...
export default function SearchMarkerLayer({
  ...
  const clusters = useMemo(
    () => clusterSearchTimelineMarkers(markers, timeToX, width, hoveredIds),
    [markers, hoveredIds, timeToX, width],
  );
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 top-0 z-[25] overflow-hidden" style={{ height }} aria-hidden="true">
      {clusters.map((cluster) => (
        <div
          key={cluster.key}
          className="absolute top-0 transition-[left] duration-200 ease-out"
